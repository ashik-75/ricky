import Image from "next/image";
import Link from "next/link";
import { CharacterType } from "./character.types";

let data =
  "data:image/jpeg;base64,";

function Character({ name, image, id }: CharacterType) {
  return (
    <div className="p-3 transition shadow-sm hover:shadow rounded-xl cursor-pointer">
      <Link href={`/anime/${id}`}>
        <div className="relative w-full h-[200px] rounded-xl overflow-hidden">
          <Image
            alt={name}
            src={image}
            placeholder="blur"
            blurDataURL={data}
            fill
            className="object-cover object-center absolute hover:scale-110 transition-all"
          />
        </div>

        <div className="mt-2">
          <h1 className="font-semibold">{name}</h1>
        </div>
      </Link>
    </div>
  );
}

export default Character;
